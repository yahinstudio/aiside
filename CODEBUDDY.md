...
Chrome. This is a hard requirement: MV3 forbids remote code and there is no `package.json`.
...
- **Run all tests:** `node tools/test_parse.js`
  ...
  - The harness `vm.runInContext`s source files using **hardcoded absolute paths**
    (`E:\aiside\common.js` etc.). Moving the repo breaks the tests until those paths are updated.
...
`sidepanel.html:12-15` loads `deepseek.js`, `kimi.js`, `common.js`, then `sidepanel.js`.
`common.js`'s `streamChat` dispatches to `window.DEEPSEEK` / `window.KIMI` and throws a
"模块未加载" error if they are absent (`common.js:183-200`). `options.html` loads the same
provider + common scripts. If you add a shared file, wire it into every HTML that needs it.

### Provider dispatch
`settings.activeProvider` is one of `deepseek | kimi | openai | gemini`. `streamChat`
(`common.js:183`) is the single choke point:
...
### One summarize run (`sidepanel.js:67`)
...
`extractPageText` (`common.js:300`) and `extractBilibili` (`common.js:398`) are serialized by
`chrome.scripting.executeScript`. They may only reference their own arguments and page globals —
**no outer-scope variables or helper imports**, or injection fails at runtime.
- Normal pages inject `extractPageText` (isolated world), size-limited by provider:
  `API_MAX_CHARS = 24000`, `DS_MAX_CHARS = 60000` (`sidepanel.js:7-10`).
...
(`deepseek.js:334-372`). Cleanup deletes the chat session after the stream.
...
upload/parse failure silently degrades to inline text (`sidepanel.js:182-206`). There is no
...
- **Always `escapeHtml` before any Markdown substitution** (`common.js:667`, `inlineHtml`).
...
- `planStreamingRender` / `renderProvisionalTail` / `splitMarkdownBlocks` render completed
  ...
  lines) so there is no layout jump when the stream completes. `sidepanel.js:226-264` diffs
  `renderedBlocks` against the DOM to replace only changed blocks.

### Storage (`chrome.storage.local`)
- `settings` — deep-merged over `DEFAULT_SETTINGS` (`common.js:7`), so adding a field needs no migration.
...
API keys never leave the machine; they are stored only here.

## Conventions

- UI strings and code comments are **Chinese**; match that style.
- Console diagnostics use the `[AiSIDE]` prefix.
- User-facing errors go through `friendlyError` (`common.js:289`) for consistent Chinese text.
